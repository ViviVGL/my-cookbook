...
describe UserMailer, type: :mailer do
  describe 'send email to a friend' do
    it 'and render email addresses and subject' do
      ...
      mail = UserMailer.email_to_friend(recipe.id, friend_email, message_to_friend)
    ...
    it 'and email body' do
      ...
      expect(mail.body).to include "#{recipe.photo.url(:medium)}"
      expect(mail.body).to include "#{recipe.cuisine.name}"
      expect(mail.body).to include "#{recipe.recipe_type.name}"
      expect(mail.body).to include "#{recipe.difficulty}"
      expect(mail.body).to include "#{recipe.cook_time}"
      expect(mail.body).to include "#{recipe.ingredients}"
      expect(mail.body).to include "#{recipe.method}"
    end
  end
end
